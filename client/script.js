...
const animalContainer = document.querySelector('body')
const ohMy = () => {
    // YOUR CODE HERE
    ...
    .then((res) => {
        ...
        for(let i=0; i<animals.length; i++) {
            ...
            newAnimal.textContent = animals[i]
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/book')
        .then((res) => {
...
// CODE HERE
const query = () => {
    axios.get('http://localhost:3000/query-test/?lions, tigers, bears')
        .then((res) =>{
            console.log(res.data)
        })
        .catch((err) =>{
...
/*
    ...
    2: Send more than 1 query on the URL -- what happened? 
        You sent query: multiple items with value ! it searches for them all together
*/
...
const createFood = (event) => {
    ...
    let foodInput = food.value
    let listContainer = document.createElement('ul')
    animalContainer.appendChild(listContainer)
